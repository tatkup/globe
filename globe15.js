...
function FsGlobe() {
  ...
  s.enableDamping = true;
  s.enablePan = false;
  s.minDistance = 1.2;
  s.maxDistance = 4;
  s.autoRotate = true;
  s.domElement.focus = null;
  s.autoRotateSpeed = 0.05;
  s.rotateSpeed = 0.5; // Initial rotate speed
  s.target.set(0, 0, 0);
  r.position.set(2.5, 0, 0);
  s.enableZoom = true;
  s.minZoom = 5;
  s.maxZoom = 25;
  s.dampingFactor = 0.75;
  s.update();
  ...
  d.add(h),
    ...
    (function () {
      ...
      for (const [n, d] of m.entries()) {
        ...
        (o.rotation.y = THREE.MathUtils.degToRad(p) + e), (i.rotation.x = THREE.MathUtils.degToRad(c) + t), r.updateWorldMatrix(true, false);
  ...
  function H() {
    ...
    if (
      ...
      (function (e) {
        ...
        return l && e.setSize(n, o, false), l;
      })(i))
    ) {
  ...
  function R() {
    p || (cancelAnimationFrame(c), (p = true), (c = requestAnimationFrame(H)));
  }
  H();
  s.addEventListener("change", R);

  // Enable zoom controls for the mouse wheel
  o.addEventListener("wheel", function (event) {
    ...
    // Update rotateSpeed based on the current zoom level
    const zoomFactor = (s.getZoomScale() - s.minZoom) / (s.maxZoom - s.minZoom);
    s.rotateSpeed = 0.5 + 0.4 * (1 - zoomFactor); // Adjust the formula as needed
    R(); // Render after zoom
  });
