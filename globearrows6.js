...
function FsGlobe() {
  ...
  s.enablePan = false;
  s.minDistance = 1.2;
  s.maxDistance = 10; // Increased max distance for further zoom
  s.autoRotate = true;
  s.domElement.focus = null;
  s.autoRotateSpeed = 0;
  s.rotateSpeed = 0.05; // Initial rotate speed
  s.target.set(0, 0, 0);
  r.position.set(2.5, 0, 0);
  s.enableZoom = true;
  s.minZoom = 5;
  s.maxZoom = 30; // Increased max zoom
...
function updateControls() {
  ...
  s.rotateSpeed = 0.5 + 0.4 * Math.pow(1 - zoomFactor, 2); // Adjust the formula as needed
}
